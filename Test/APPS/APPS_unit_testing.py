...
def parse_args():
    ...
    parser.add_argument("-v", "--verbosity-level", action="store", type=int,
            help="")
    parser.add_argument("-s", "--source",  type=str, default=r"C:\Users\harsh\Desktop\NLP\solutions\testset_300", 
            choices=[r"C:\Users\harsh\Desktop\NLP\solutions\testset_300","train"],
            help="which data source to gather from.")
    parser.add_argument("-d", "--data",  type=str, default="question",
            choices=["question", "q", "solutions", "sol", "s", "starter", "test", "t"],
            help="which type of data to receive.")
    parser.add_argument("-n", "--number",  type=int, default=0,help="which problem to query.")
    parser.add_argument("--start_number", type=int, default=0,help="which problem to start with.")
    parser.add_argument("--end_number",type=int, default=100,help="which problem to query.")
    args = parser.parse_args()
    return args


def get_valid_problems(data_dir="test"):
...
def run_test2(prob_path:str=None, problem_list:List[str]=None, prob_index:int=None, 
        test:str=None, debug:bool=False):
    """
    if test is not None it'll try to run the code.
    otherwise it'll just return an input and output pair.
        ...
        if which_type == CODE_TYPE.call_based:
            ...
            try:
                ...
                if "class Solution" not in test:
                    ...
                    tmp = tmp_sol.Solution()
                print(40000000000000000000000000000000000000000000000)
                print(tmp_sol)
                a = input()
        ...
        #        signal.alarm(0)
         ...
         #       signal.alarm(0)
          ...
          #  signal.alarm(0)
            ...
            # sol
            print(20000000000000000000000000000000000000000000000)
            tmp_test = test.split("\n")
...
def main(args):
    ...
    #     print("starter", tmp)
        ...
        tmp = run_test(None,problem_list, prob_index, test=sols[0],debug=False)
        for i in tmp:
            if i != True:
                record.append([args.number,tmp])
                break   
        print("results = ", tmp)
...
if __name__ == "__main__":
    ...
    # Convert the dictionary to a JSON object with indentation
    json_object = json.dumps(dictionary, indent=4)

    # Write the JSON object to a file
    with open("last_file.json", "w") as outfile:
