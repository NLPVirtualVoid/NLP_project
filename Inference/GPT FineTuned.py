...
# All paths are relative to where the script is located
...
prompts=[]
print_data = False
data_set = "GPT_FT" # test or train
question_root = script_directory+"/testset_300/"
output_root = script_directory+"/Output/testset_300/"+data_set

#Hendryks model fine tuned on APPS training data
#model_path = script_directory+"/apps-main/models/1.5B"
# model_path = script_directory+"/gpt_fine_tuned_model"
model_path = script_directory+"/gpt_fine_tuned_model"
...
if print_data: print(f"Pipeline time: {(pipeline_end_time - pipeline_start_time):.1f}")


# loop over questions populating prompts list
for i in number_strings:
        # read in question
        question_path = question_root+"/"+i+"/question.txt"
        ...
        if os.path.isfile(question_root+"/"+i+"/starter_code.py"):
                with open(question_root+"/"+i+"/starter_code.py", "r") as file:
